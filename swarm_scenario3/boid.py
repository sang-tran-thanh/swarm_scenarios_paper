...
###   Reynolds rules + custom rule (avoid) with dynamic weighting factors   #####
    ...
    def checkcolision_neigh(self,boids):
        if boids:
            for boid in boids:
                if self.pos.distance_to(boid.pos)< 7 and boid != self:
                    return True
        else:
            return False
    ...
    def checkcolision_ob(self,obs):
        if obs:
            for ob in obs:
                if self.pos.distance_to(ob.pos)< 7:
                    return True
        else:
            return False
